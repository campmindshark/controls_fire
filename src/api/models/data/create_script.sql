...
CREATE TABLE tblGPIO ( gpio_id integer PRIMARY KEY,
                                               active_low integer, direction text, edge text, power text);
...
CREATE TABLE tblParts
  ( part_id integer PRIMARY KEY
    , gpio_id integer
    , type text,
   FOREIGN KEY (gpio_id) REFERENCES tblGPIO (gpio_id) ON DELETE
   SET NULL);
...
CREATE TABLE tblLinkSourceParts
  ( link_id integer PRIMARY KEY,
   ...
   FOREIGN KEY (source_id) REFERENCES tblParts (part_id) ON DELETE CASCADE
   FOREIGN KEY (part_id) REFERENCES tblParts (part_id) ON DELETE CASCADE);
